...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//hapus data dengan kofiirmasi
		$("tbl_data").on('click','.btn_hapus',function(){
			var noinduk = $(this).attr('data-id');
			var status  = confirm('Yakin ingin dihapus');
			if(status){
				$.ajax({
	...
	//menambahkan data kedalamd atabase
	$("btn_add_data").on('click',function(){
		var noinduk=$('input[name="noinduk"]').val();
		var nama=$('input[name="nama"').val();
		var alamaat=$('input[name="alamat"').val();
		var hobi=$('input[name="hobi"').val();
		$.ajax({
			url : '<?php echo base_url()?> Siswa/tambahdata',
			success: function(response){
	...
	$("#tbl_data").on('click','btn_edit',function(){
		var noinduk = $(this).attr('data-id');
		$.ajax({
			url: '<?php echo base_url(); ?> Siswa/ambildatabynoinduk',
			type: 'POST',
			data: {noinduk:noinduk},
			dataType: 'json',
			success: function(response){
			...
			$('input[name="alamat"').val(response[0].alamat);
			$('input[name="hobi"').val(response[0]).hobi;
		}
		})
	});
	...
	$("$btn_update_data").on('click',function(){
		...
		$.ajax({
			url: '<?php echo base_url()?>siswa/perbaruidata',
			type: 'POST',
			data: {noinduk:noinduk,nama:nama,alamat:alamaat,hobi:hobi},
			success: function(response){
